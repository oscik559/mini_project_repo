...
class UserInterface:
    def __init__(self):
        ...
        # Buttons
        tk.Label(self.root, text="Multi_modal HRI System", font=("Arial", 16)).pack(
            pady=10
        )

        tk.Button(
            self.root, text="Create Database", command=self.create_database, width=25
        ).pack(pady=5)
        tk.Button(
            self.root,
            text="Activate Voice Processing",
            command=self.activate_voice_processing,
            width=25,
        ).pack(pady=5)
        tk.Button(
            self.root,
            text="Activate Video Processing",
            command=self.activate_video_processing,
            width=25,
        ).pack(pady=5)
        tk.Button(
            self.root,
            text="Run LLM Processing",
            command=self.run_llm_processing,
            width=25,
        ).pack(pady=5)
        tk.Button(
            self.root,
    ...
    def activate_video_processing(self):
        ...
        processor = VideoProcessor(
            update_status_callback=lambda msg: self.status_label.config(text=msg)
        )
