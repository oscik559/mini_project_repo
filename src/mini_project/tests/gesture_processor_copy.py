...
class GestureDetector:
    ...
    #                                  (timestamp, gesture_type, gesture_text, natural_description, confidence, hand_label)
    #                                  VALUES (?, ?, ?, ?, ?, ?, ?)""",
    #                 (
    #                     timestamp,
    #                     gesture_type,
    ...
    def _log_gesture(self, gesture_data: Dict[str, Any]):
        """Send to unifier instead of direct DB access"""
        self.unifier.add_command(
            {
                "modality": "gesture",
                "timestamp": datetime.now().isoformat(),
                "command_type": gesture_data["gesture_type"],
                "command_text": gesture_data["gesture_text"],
                "natural_description": gesture_data["natural_description"],
                "confidence": gesture_data["confidence"],
                "hand_label": gesture_data["hand_label"],
            }
        )
        ...
        if self.frame_counter % self.frame_skip == 0:
            ...
            if self.last_detection:
                for idx, d in enumerate(self.last_detection):
                    ...
                    self._log_gesture(
                        {
                            "gesture_type": d["gesture"],
                            "gesture_text": d["gesture_text"],
                            "natural_description": d["description"],
                            "confidence": d["confidence"],
                            "hand_label": d["hand_label"],
                        }
                    )
