...
def process_small_tray(
    ...
    red_mask = cv2.inRange(hsv_image, (0, 100, 100), (10, 255, 255)) | cv2.inRange(
        hsv_image, (160, 100, 100), (180, 255, 255)
    )
    ...
    red_contours, _ = cv2.findContours(
        red_within_tray, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
    )
    if red_contours:
        ...
        edge_mid = (
            (tray_points[0][0] + tray_points[3][0]) // 2,
        ...
        if screw_center_orange is not None:
            relative_vector_2 = depth_image_process(
                edge_mid, screw_center_orange, depth_frame, intrinsics, scaling_factor
            )
