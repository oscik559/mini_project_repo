...
class MainApplication:
    def __init__(self, root):
        ...
        title_label = tk.Label(
            root, text="Welcome Home!", font=("Garamond", 24, "bold")
        )
        ...
        self.create_hover_button(
            root, text="Login", bg="#2ecc71", fg="white", command=self.open_login_window
        ).pack(pady=20)
        self.create_hover_button(
            root, text="Exit", bg="#e74c3c", fg="white", command=self.root.quit
        ).pack(pady=20)

        # Footer
        footer_label = tk.Label(
            root, text="Linköping University", font=("Arial", 10), fg="gray"
        )
    ...
    def create_hover_button(self, parent, text, bg, fg, command):
        ...
        button.configure(
            borderwidth=2, highlightbackground=bg, highlightcolor=bg, relief="ridge"
        )
        return button
        ...
        def register_user():
            ...
            if first_name and last_name and liu_id and email and password:
                try:
                    ...
                    cursor = conn.cursor()
                    cursor.execute(
                        "INSERT INTO users (first_name, last_name, liu_id, email, password) VALUES (?, ?, ?, ?, ?)",
    ...
    def open_login_window(self):
        ...
        tk.Button(
            login_window, text="Login", bg="#3498db", fg="white", command=login_user
        ).pack(pady=20)
